...
public:
    ...
    friend std::istream& operator>>(std::istream& ifs, Vector<T> v);
    friend std::ostream& operator<<(std::ostream& ofs, const Vector<T> v);
    ~Vector();
};
...
template <typename T>
std::istream& operator>>(std::istream& ifs, Vector<T> v)
{
    ifs >> v.size >> v.capacity;
...
template <typename T>
std::ostream& operator<<(std::ostream& ofs, const Vector<T> v) {
    ofs << v.size << v.capacity;
    for (int i = 0; i < v.size; i++) {
        ofs >> v.data[i];
    }
    return ofs;
